...
const USE_PDF_PRINT =
  (process.env.USE_PDF_PRINT || "false").toLowerCase() === "true";
...
if (!JWT_SECRET || !REFRESH_SECRET) {
  logger.fatal(
    "FATAL ERROR: JWT_SECRET and REFRESH_SECRET must be defined in .env"
  );
  process.exit(1);
}
...
async function processReminders() {
  ...
  try {
    ...
    for (const reminder of reminders) {
      ...
      if (!memberEmail) {
        logger.warn(
          `Skipping reminder for member ${reminder.memberId}: no email address found.`
        );
        continue;
      }
      ...
      const html = `
                ...
                <p>This is a reminder that your payment of ${
                  reminder.currency.symbol
                }${
        reminder.amount
      } is due on ${reminder.dueDate.toLocaleDateString()}.</p>
      ...
      if (emailSent) {
        ...
        logger.info(
          `Reminder email sent for payment ID: ${reminder.id} to ${memberEmail}`
        );
      } else {
        await prisma.paymentReminder.update({
          where: { id: reminder.id },
        ...
        logger.error(
          `Failed to send reminder email for payment ID: ${reminder.id}`
        );
      }
    }
  } catch (error) {
...
const createRoleSchema = Joi.object({
  ...
  permissions: Joi.object({
    users: Joi.array()
      .items(
        Joi.string().valid("read", "create", "update", "delete", "lock_unlock")
      )
      .default([]),
    roles: Joi.array()
    ...
    transactions: Joi.array()
      .items(Joi.string().valid("read", "create", "update", "delete", "refund"))
      .default([]),
    reports: Joi.array()
      .items(Joi.string().valid("read", "export", "advanced"))
      .default([]),
    settings: Joi.array()
      .items(Joi.string().valid("read", "update", "system_config"))
      .default([]),
    revenue_heads: Joi.array()
      .items(Joi.string().valid("read", "create", "update", "delete"))
      .default([]),
    expenditure_heads: Joi.array()
      .items(Joi.string().valid("read", "create", "update", "delete"))
      .default([]),
    currencies: Joi.array()
      .items(Joi.string().valid("read", "manage"))
      .default([]),
    ...
    projects: Joi.array()
      .items(Joi.string().valid("read", "create", "update", "delete"))
      .default([]),
    expenditures: Joi.array()
      .items(
        Joi.string().valid("read", "create", "update", "delete", "approve")
      )
      .default([]),
    assets: Joi.array()
      .items(Joi.string().valid("read", "create", "update", "delete"))
...
const updateRoleSchema = createRoleSchema.fork(
...
const memberCreateSchema = Joi.object({
  ...
  ageCategory: Joi.string()
    .valid("ADULT", "YOUTH", "CHILD", "ELDERLY")
    .default("ADULT"),
  phoneNumber: Joi.string().trim().max(20).allow("", null),
  email: Joi.string().email().allow("", null),
...
const memberUpdateSchema = memberCreateSchema.fork(
  Object.keys(memberCreateSchema.describe().keys),
  (schema) => schema.optional()
);
...
const projectUpdateSchema = projectCreateSchema.fork(
  Object.keys(projectCreateSchema.describe().keys),
  (schema) => schema.optional()
);
...
const expenditureCreateSchema = Joi.object({
  ...
  recurringUntil: Joi.date().allow(null),
  ...
  processedBy: Joi.any().forbidden(),
});

const expenditureUpdateSchema = expenditureCreateSchema.fork(
  Object.keys(expenditureCreateSchema.describe().keys),
  (schema) => schema.optional()
);
...
const supplierUpdateSchema = supplierCreateSchema.fork(
  Object.keys(supplierCreateSchema.describe().keys),
  (schema) => schema.optional()
);
...
const assetCreateSchema = Joi.object({
  ...
  condition: Joi.string()
    .valid("EXCELLENT", "GOOD", "FAIR", "POOR", "DAMAGED", "OBSOLETE")
    .default("EXCELLENT"),
...
const assetUpdateSchema = assetCreateSchema.fork(
  Object.keys(assetCreateSchema.describe().keys),
  (schema) => schema.optional()
);
...
const contractUpdateSchema = contractCreateSchema.fork(
  Object.keys(contractCreateSchema.describe().keys),
  (schema) => schema.optional()
);
...
const budgetPeriodCreateSchema = Joi.object({
  ...
  budgetType: Joi.string()
    .valid("ANNUAL", "QUARTERLY", "MONTHLY", "PROJECT_BASED", "EVENT_BASED")
    .default("ANNUAL"),
  totalBudget: Joi.number().positive().precision(2).required(),
  currencyCode: Joi.string().length(3).default("USD"),
});

const budgetPeriodUpdateSchema = budgetPeriodCreateSchema.fork(
  Object.keys(budgetPeriodCreateSchema.describe().keys),
  (schema) => schema.optional()
);
...
async function generateReceiptNumber(type, branchCode, tx) {
  ...
  const prefix =
    type === "contribution" ? "MC" : type === "transaction" ? "TR" : "EX";
  ...
  if (lastRecord) {
    const receiptNum =
      type === "expenditure"
        ? lastRecord.voucherNumber
        : lastRecord.receiptNumber;
    if (receiptNum) {
  ...
  return `${branchCode}-${prefix}-${currentYear}-${String(
    nextSequence
  ).padStart(6, "0")}`;
}
...
(async () => {
  try {
    ...
    if (shouldInitialize) {
      ...
      await Promise.all(
        defaultRevenueHeads.map(async (head, index) => {
          const code = `${DEFAULT_ADMIN_BRANCH}R${String(index + 1).padStart(
            3,
            "0"
          )}`;
      ...
      await Promise.all(
        defaultExpenditureHeads.map(async (head, index) => {
          const code = `${DEFAULT_ADMIN_BRANCH}E${String(index + 1).padStart(
            3,
            "0"
          )}`;
      ...
      const roles = await prisma.role.findMany({
        where: { name: { in: rolesToEnsure } },
      });
      for (const role of roles) {
        const permissions = role.permissions || {};
        permissions.currencies = Array.isArray(permissions.currencies)
          ? permissions.currencies
          : [];
        permissions.payment_methods = Array.isArray(permissions.payment_methods)
        ...
        if (changed) {
          ...
          logger.info(
            `Updated role permissions for ${role.name}: added read on currencies/payment_methods`
          );
        }
      }
    }
    ...
    logger.fatal(
      { error },
      "Failed to connect to DB or perform initial setup."
    );
    process.exit(1);
  }
})();
  ...
  try {
    ...
    for (const role of roles) {
      ...
      if (changed) {
        ...
        logger.info(
          `Ensured read permissions for ${role.name} on currencies/payment_methods`
        );
      }
    }
  } catch (e) {
...
app.post("/print-receipt", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Printing failed", error: err.message });
  }
});
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (user.locked)
        throw new ForbiddenError("Account locked. Contact administrator.");
      if (!user.isActive)
        throw new ForbiddenError("Account is inactive. Contact administrator.");
      if (!user.role || !user.role.isActive)
        throw new ForbiddenError("User role is invalid or inactive.");
      ...
      await prisma.$transaction(async (tx) => {
        if (!isPasswordValid) {
          ...
          if (newAttempts >= MAX_LOGIN_ATTEMPTS) {
            ...
            msg =
              "Account locked due to multiple failed attempts. Contact administrator.";
          }
  ...
  asyncHandler(async (req, res) => {
    ...
    const newUser = await prisma.user.create({
      ...
      select: {
        ...
        phoneNumber: true,
        role: {
          // Include role details in response
          select: {
            id: true,
            name: true,
          },
        ...
        branch: {
          // Include branch details in response
          select: {
            code: true,
            name: true,
          },
    ...
    console.log(
      `USER CREATED: ${newUser.username} (${roleExists.name}) at ${branchExists.name} by ${req.user.username}`
    );
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      req.user.id !== userId &&
      !req.user.role.permissions.users?.includes("read")
    ) {
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      req.user.id !== userId &&
      !req.user.role.permissions.users?.includes("update")
    ) {
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingRole._count.users > 0)
      throw new ConflictError("Cannot delete role that has users assigned.");
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingBranch) {
      ...
      if (existingBranch.name === name)
        throw new ConflictError("Branch name already exists.");
    }
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const hasDependencies = Object.values(existingBranch._count).some(
      (count) => count > 0
    );
    if (hasDependencies) {
      throw new ConflictError(
        "Cannot delete branch with associated records (users, members, projects, etc.)."
      );
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingMember)
      throw new ConflictError("Member number already exists.");
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      member.branchCode !== req.user.branchCode &&
      !req.user.role.permissions.members?.includes("read_all")
    ) {
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      existingMember.branchCode !== req.user.branchCode &&
      !req.user.role.permissions.members?.includes("update_all")
    ) {
      throw new ForbiddenError(
        "You can only update members in your own branch."
      );
    }

    if (
      value.memberNumber &&
      value.memberNumber !== existingMember.memberNumber
    ) {
      const conflict = await prisma.member.findUnique({
        where: { memberNumber: value.memberNumber },
      });
      if (conflict)
        throw new ConflictError("New member number already exists.");
    }
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      existingMember.branchCode !== req.user.branchCode &&
      !req.user.role.permissions.members?.includes("delete_all")
    ) {
      throw new ForbiddenError(
        "You can only delete members in your own branch."
      );
    }

    const hasDependencies =
      existingMember._count.contributions > 0 ||
      existingMember._count.generalTransactions > 0 ||
      existingMember._count.memberProjects > 0;
    if (hasDependencies) {
      throw new ConflictError(
        "Cannot delete member with associated contributions or transactions."
      );
    }
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      !req.user.role.permissions.projects?.includes("read_all")
    ) {
      throw new ForbiddenError(
        "You can only view projects in your own branch."
      );
    }
    ...
    res.status(200).json({
      ...
      progressPercentage:
        project.targetAmount > 0
          ? Math.min(
              100,
              (totalCollected / parseFloat(project.targetAmount)) * 100
            )
          : 0,
    });
  })
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      !req.user.role.permissions.projects?.includes("update_all")
    ) {
      throw new ForbiddenError(
        "You can only update projects in your own branch."
      );
    }

    if (value.branchCode) {
      const branchExists = await prisma.branch.findUnique({
        where: { code: value.branchCode },
      });
      if (!branchExists) throw new NotFoundError("New branch not found.");
    }
    if (value.currencyCode) {
      const currencyExists = await prisma.currency.findUnique({
        where: { code: value.currencyCode },
      });
...
app.post(
  "/api/members/:memberId/projects",
  authenticateToken,
  checkPermission("members", "update"),
  asyncHandler(async (req, res) => {
    const memberId = parseInt(req.params.memberId, 10);
    const { error, value } = memberProjectSchema.validate({
      ...req.body,
      memberId,
    });
    if (error) throw new ValidationError(error.details[0].message);
    ...
    if (
      ...
      throw new ForbiddenError(
        "You can only enroll members in your own branch."
      );
    }
    ...
    if (existingEnrollment)
      throw new ConflictError(
        "Member is already associated with this project."
      );
  ...
  asyncHandler(async (req, res) => {
    ...
    if (startDate || endDate) {
      ...
      if (endDate) {
        ...
        whereClause.transactionDate.lt = new Date(
    ...
    try {
      ...
      throw new AppError(
        "Failed to fetch transactions due to a database error.",
        500
      );
    }
  })
...
// Create Expenditure
// ... existing server code before pp.post

app.post(
  "/api/expenditures",
  authenticateToken,
  checkPermission("expenditures", "create"),
  asyncHandler(async (req, res) => {
    ...
    const [expenditureHead, currency, paymentMethod, branch] =
      await Promise.all([
        prisma.expenditureHead.findUnique({
          where: { code: value.expenditureHeadCode },
        }),
        prisma.currency.findUnique({ where: { code: value.currencyCode } }),
    ...
    if (!expenditureHead)
      throw new NotFoundError("Expenditure head not found.");
    if (!currency) throw new NotFoundError("Currency not found.");
    if (!paymentMethod) throw new NotFoundError("Payment method not found.");
    if (!branch) throw new NotFoundError("Branch not found.");

    // Verify optional member relations (reimbursedTo)
    if (value.reimbursedTo) {
      // FIX: Corrected variable name from 'approver' to 'reimbursee' and error message
      const reimbursee = await prisma.member.findUnique({
        where: { id: value.reimbursedTo },
      });
...
// ... existing server code after pp.post
// Get Expenditures
app.get(
  "/api/expenditures",
  authenticateToken,
  checkPermission("expenditures", "read"),
  asyncHandler(async (req, res) => {
    const {
      limit = 50,
      offset = 0,
...
app.patch(
  "/api/expenditures/:id",
  authenticateToken,
  checkPermission("expenditures", "update"),
  asyncHandler(async (req, res) => {
    const expenditureId = parseInt(req.params.id, 10);
    if (isNaN(expenditureId))
      throw new ValidationError("Invalid expenditure ID.");
    ...
    if (
      existingExpenditure.branchCode !== req.user.branchCode &&
      !req.user.role.permissions.expenditures?.includes("update_all")
    ) {
      throw new ForbiddenError(
        "You can only update expenditures in your own branch."
      );
    }

    if (
      existingExpenditure.approvalStatus !== "PENDING" &&
      !req.user.role.permissions.expenditures?.includes("approve")
    ) {
      throw new ForbiddenError(
        "Only approvers can modify approved expenditures."
      );
    }

    if (value.amount || value.taxAmount) {
      value.totalAmount =
        parseFloat(value.amount || existingExpenditure.amount) +
        parseFloat(value.taxAmount || existingExpenditure.taxAmount);
    }
...
app.delete(
  "/api/expenditures/:id",
  authenticateToken,
  checkPermission("expenditures", "delete"),
  asyncHandler(async (req, res) => {
    const expenditureId = parseInt(req.params.id, 10);
    if (isNaN(expenditureId))
      throw new ValidationError("Invalid expenditure ID.");
    ...
    if (
      existingExpenditure.branchCode !== req.user.branchCode &&
      !req.user.role.permissions.expenditures?.includes("delete_all")
    ) {
      throw new ForbiddenError(
        "You can only delete expenditures in your own branch."
      );
    }

    if (
      existingExpenditure._count.receipts > 0 ||
      existingExpenditure._count.assetCreated > 0
    ) {
      throw new ConflictError(
        "Cannot delete expenditure with associated receipts or assets."
      );
    }
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const hasDependencies =
      existingSupplier._count.expenditures > 0 ||
      existingSupplier._count.contracts > 0 ||
      existingSupplier._count.evaluations > 0;
    if (hasDependencies) {
      throw new ConflictError(
        "Cannot delete supplier with associated expenditures, contracts, or evaluations."
      );
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (value.expenditureId) {
      const expenditureExists = await prisma.expenditure.findUnique({
        where: { id: value.expenditureId },
      });
      ...
      return tx.asset.create({
        data: {
          ...
          depreciationRate: value.depreciationRate
            ? new Prisma.Decimal(value.depreciationRate)
            : null,
        },
        include: {
          branch: { select: { name: true } },
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      value.expenditureId &&
      value.expenditureId !== existingAsset.expenditureId
    ) {
      const expenditureExists = await prisma.expenditure.findUnique({
        where: { id: value.expenditureId },
      });
      if (!expenditureExists)
        throw new NotFoundError("New expenditure not found.");
    }
    ...
    const updatedAsset = await prisma.asset.update({
      ...
      data: {
        ...
        purchasePrice: value.purchasePrice
          ? new Prisma.Decimal(value.purchasePrice)
          : undefined,
        currentValue: value.currentValue
          ? new Prisma.Decimal(value.currentValue)
          : undefined,
        depreciationRate: value.depreciationRate
          ? new Prisma.Decimal(value.depreciationRate)
          : undefined,
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingAsset._count.maintenanceRecords > 0) {
      throw new ConflictError(
        "Cannot delete asset with associated maintenance records."
      );
    }
    ...
    const updatedContract = await prisma.contract.update({
      ...
      data: {
        ...
        contractValue: value.contractValue
          ? new Prisma.Decimal(value.contractValue)
          : undefined,
      },
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingContract._count.milestones > 0) {
      throw new ConflictError(
        "Cannot delete contract with associated milestones."
      );
    }
...
app.post(
  "/api/budget-periods",
  authenticateToken,
  checkPermission("budgets", "create"),
  asyncHandler(async (req, res) => {
...
app.patch(
  ...
  authenticateToken,
  checkPermission("budgets", "update"),
  asyncHandler(async (req, res) => {
    const budgetPeriodId = parseInt(req.params.id, 10);
    if (isNaN(budgetPeriodId))
      throw new ValidationError("Invalid budget period ID.");
    ...
    if (
      value.currencyCode &&
      value.currencyCode !== existingPeriod.currencyCode
    ) {
      const currencyExists = await prisma.currency.findUnique({
        where: { code: value.currencyCode },
      });
    ...
    const updatedPeriod = await prisma.budgetPeriod.update({
      ...
      data: {
        ...
        totalBudget: value.totalBudget
          ? new Prisma.Decimal(value.totalBudget)
          : undefined,
        actualSpent: value.actualSpent
          ? new Prisma.Decimal(value.actualSpent)
          : undefined,
      },
...
app.delete(
  "/api/budget-periods/:id",
  authenticateToken,
  checkPermission("budgets", "delete"),
  asyncHandler(async (req, res) => {
    const budgetPeriodId = parseInt(req.params.id, 10);
    if (isNaN(budgetPeriodId))
      throw new ValidationError("Invalid budget period ID.");
    ...
    if (existingPeriod._count.budgetLines > 0) {
      throw new ConflictError(
        "Cannot delete budget period with associated budget lines."
      );
    }
...
app.post(
  "/api/budget-periods/:budgetPeriodId/lines",
  authenticateToken,
  checkPermission("budgets", "create"),
  asyncHandler(async (req, res) => {
    ...
    const [budgetPeriod, expenditureHead] = await Promise.all([
      ...
      prisma.expenditureHead.findUnique({
        where: { code: expenditureHeadCode },
      }),
    ]);
    ...
    res
      .status(201)
      .json({ message: "Budget line saved successfully.", budgetLine });
  })
);
...
app.post(
  "/api/currencies",
  authenticateToken,
  checkPermission("currencies", "manage"),
  asyncHandler(async (req, res) => {
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!existingCurrency) throw new NotFoundError("Currency not found.");
    const hasDependencies = Object.values(existingCurrency._count).some(
      (count) => count > 0
    );
    if (hasDependencies) {
      throw new ConflictError(
        "Cannot delete currency with associated records (transactions, projects, etc.)."
      );
    }
...
app.post(
  "/api/payment-methods",
  authenticateToken,
  checkPermission("payment_methods", "manage"),
  asyncHandler(async (req, res) => {
...
app.get(
  "/api/payment-methods",
  authenticateToken,
  checkPermission("payment_methods", "read"),
  asyncHandler(async (req, res) => {
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!existingMethod) throw new NotFoundError("Payment method not found.");
    const hasDependencies = Object.values(existingMethod._count).some(
      (count) => count > 0
    );
    if (hasDependencies) {
      throw new ConflictError(
        "Cannot delete payment method with associated transactions or expenditures."
      );
    }
...
app.post(
  "/api/revenue-heads",
  authenticateToken,
  checkPermission("revenue_heads", "create"),
  asyncHandler(async (req, res) => {
    ...
    if (existingHead)
      throw new ConflictError(
        "Revenue head with this name already exists for this branch."
      );
...
app.get(
  "/api/revenue-heads",
  authenticateToken,
  checkPermission("revenue_heads", "read"),
  asyncHandler(async (req, res) => {
...
// Update Revenue Head
app.patch(
  "/api/revenue-heads/:code",
  authenticateToken,
  checkPermission("revenue_heads", "update"),
  asyncHandler(async (req, res) => {
    const { code } = req.params;
    const { error, value } = revenueHeadSchema.validate(req.body);
    if (error) throw new ValidationError(error.details[0].message);
    ...
    if (value.name && value.name !== existingHead.name) {
      const nameConflict = await prisma.revenueHead.findFirst({
        where: {
          name: value.name,
          branchCode: existingHead.branchCode,
          NOT: { code },
        },
      });
      if (nameConflict)
...
app.delete(
  "/api/revenue-heads/:code",
  authenticateToken,
  checkPermission("revenue_heads", "delete"),
  asyncHandler(async (req, res) => {
    const { code } = req.params;
    ...
    const existingHead = await prisma.revenueHead.findUnique({
      where: { code },
      include: {
        _count: { select: { transactions: true, contributionPlans: true } },
      },
    });

    if (!existingHead) throw new NotFoundError("Revenue head not found.");
    const hasDependencies =
      existingHead._count.transactions > 0 ||
      existingHead._count.contributionPlans > 0;
    if (hasDependencies) {
      throw new ConflictError(
        "Cannot delete revenue head with associated transactions or plans."
      );
    }
...
app.post(
  ...
  authenticateToken,
  checkPermission("expenditure_heads", "create"),
  asyncHandler(async (req, res) => {
    ...
    if (existingHead)
      throw new ConflictError(
        "Expenditure head with this name already exists for this branch."
      );
...
app.get(
  "/api/expenditure-heads",
  authenticateToken,
  checkPermission("expenditure_heads", "read"),
  asyncHandler(async (req, res) => {
...
app.patch(
  "/api/expenditure-heads/:code",
  authenticateToken,
  checkPermission("expenditure_heads", "update"),
  asyncHandler(async (req, res) => {
    const { code } = req.params;
    const { error, value } = expenditureHeadSchema.validate(req.body);
    if (error) throw new ValidationError(error.details[0].message);
    ...
    const updatedHead = await prisma.expenditureHead.update({
      ...
      data: {
        ...
        budgetLimit: value.budgetLimit
          ? new Prisma.Decimal(value.budgetLimit)
          : undefined,
      },
    });
...
app.delete(
  "/api/expenditure-heads/:code",
  authenticateToken,
  checkPermission("expenditure_heads", "delete"),
  asyncHandler(async (req, res) => {
    const { code } = req.params;
    ...
    const existingHead = await prisma.expenditureHead.findUnique({
      where: { code },
      include: {
        _count: { select: { expenditures: true, budgetLines: true } },
      },
    });

    if (!existingHead) throw new NotFoundError("Expenditure head not found.");
    const hasDependencies =
      existingHead._count.expenditures > 0 ||
      existingHead._count.budgetLines > 0;
    if (hasDependencies) {
      throw new ConflictError(
        "Cannot delete expenditure head with associated expenditures or budget lines."
      );
    }
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Get total amounts
    const [contributionTotal, transactionTotal, expenditureTotal] =
      await Promise.all([
    ...
    res.status(200).json({
      ...
      totalCollected: {
        ...
        total:
          parseFloat(contributionTotal._sum.amount || 0) +
          parseFloat(transactionTotal._sum.amount || 0),
      },
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.info(
      `Password reset token generated for user: ${username}. Token: ${token.substring(
        0,
        5
      )}...`
    );
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.info(
      `Password successfully reset for user: ${resetEntry.user.username}`
    );
...
app.get("/qz/certificate", (req, res) => {
  if (!certificate) {
    return res.status(404).json({
      error:
        "Certificate not found. Please ensure QZ Tray certificates are properly configured.",
    });
  }
  ...
  if (err instanceof Prisma.PrismaClientKnownRequestError) {
    ...
    switch (err.code) {
      ...
      case "P1000":
        statusCode = 500;
        message =
          "A database authentication error occurred. Please check your credentials.";
        break;
      case "P1001":
        statusCode = 503;
        message =
          "Cannot reach the database server. It might be down or not configured correctly.";
        break;
      case "P1008":
        statusCode = 408;
